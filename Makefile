...
NETWORK_ARGS_READ := --rpc-url http://localhost:8545 --private-key $(DEFAULT_ANVIL_KEY) 
...
setGenerator:
	@forge script script/Interactions.s.sol:setGeneratorAuction --sender $(SENDER_ADDRESS) $(NETWORK_ARGS)

getGenerator:
	@forge script script/Interactions.s.sol:getGeneratorAuction --sender $(SENDER_ADDRESS) $(NETWORK_ARGS_READ)

getParam:
	@forge script script/Interactions.s.sol:getParam --sender $(SENDER_ADDRESS) $(NETWORK_ARGS_READ)